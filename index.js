...
const GEMINI_API_VER = process.env.GEMINI_API_VERSION || 'v1beta';
...
async function askAI(system, user, wantJson) {
  ...
  const j = await geminiFetch('/models/' + encodeURIComponent(model) + ':generateContent', payload);
  const c = j.candidates && j.candidates[0];
  if (!c) { const e = new Error('ai_empty'); e.code = 'ai_empty'; throw e; }
  return ((c.content && c.content.parts) || []).map((p) => p.text || '').join('');
}

/* --- Gemini: so'rov yuborish (kalit header orqali — yangi AQ. format uchun ham) --- */
async function geminiFetch(pathPart, payload) {
  const url = GEMINI_BASE + '/' + GEMINI_API_VER + pathPart;
  const opts = {
  ...
  return getJson(url, opts, 60000);
}

/* --- Ishlaydigan modelni avtomatik topish --- */
...
let pickingModel = null;

async function pickGeminiModel() {
  if (pickedModel) return pickedModel;
  if (pickingModel) return pickingModel;
  pickingModel = (async () => {
    // 1) ListModels orqali haqiqatdan mavjud modellarni olamiz
    let available = [];
    try {
      const j = await geminiFetch('/models');
      available = (j.models || [])
        .filter((m) => (m.supportedGenerationMethods || []).includes('generateContent'))
        .map((m) => String(m.name || '').replace(/^models\//, ''));
      if (available.length) console.log('📋 Mavjud modellar:', available.slice(0, 8).join(', '));
    } catch (e) {
      console.error('⚠️  ListModels xato:', e.message);
    }

    // 2) Avval .env dagi model, keyin nomzodlar
    const wanted = [GEMINI_MODEL, ...MODEL_CANDIDATES];
    if (available.length) {
      for (const w of wanted) {
        if (available.includes(w)) { pickedModel = w; break; }
      }
      // hech biri mos kelmasa — ro'yxatdagi birinchi flash modelni olamiz
      if (!pickedModel) {
        pickedModel = available.find((m) => m.includes('flash')) || available[0];
      }
    } else {
      pickedModel = GEMINI_MODEL;
    }

    console.log('✅ Tanlangan model:', pickedModel);
    pickingModel = null;
    return pickedModel;
  })();
  return pickingModel;
  ...
  if (pathname.startsWith('/api/ai/')) {
    ...
    if (m === 'POST' && pathname === '/api/ai/chat') {
      ...
      try { answer = await askAI(sys, (ctx ? ctx + '\n\n' : '') + 'O\'quvchi: ' + text); }
        ...
        console.error('❌ AI XATO (chat):', e.code || '', e.message);
        return json(res, 502, { error: e.code || 'ai_error' });
      }
...
server.listen(PORT, () => {
  ...
  if (AI_ON && AI_PROVIDER === 'gemini') {
    setTimeout(async () => {
      try {
        const t = await askAI('Qisqa javob ber.', 'salom');
        console.log('🧪 AI test OK:', clean(t, 60));
      } catch (e) {
        console.error('🧪 AI test XATO:', e.code || '', e.message);
      }
    }, 2000);
  }
});
