...
/* --- Gemini: so'rov yuborish (header orqali — AQ. format uchun) --- */
async function geminiFetch(pathPart, payload) {
  const url = GEMINI_BASE + '/v1beta' + pathPart;
  const opts = {
...
async function pickGeminiModel() {
  ...
  const PREFER = [GEMINI_MODEL, 'gemini-flash-latest', 'gemini-2.0-flash', 'gemini-flash-lite-latest'];
  try {
    ...
    if (!pickedModel) pickedModel = available.find((m) => m.includes('flash')) || GEMINI_MODEL;
  } catch {
    pickedModel = GEMINI_MODEL;
  }
  console.log('✅ AI model:', pickedModel);
  return pickedModel;
}
...
async function askJson(system, user) {
  const raw = await askAI(system + ' Faqat JSON qaytar, hech qanday izoh yozma.', user, false);
  const j = pickJson(raw);
  if (!j) {
    console.error('❌ JSON parse xato. Raw:', String(raw).slice(0, 200));
    const e = new Error('ai_parse'); e.code = 'ai_parse'; throw e;
  }
  return j;
}
...
async function api(req, res, pathname, url, body) {
  ...
  if (pathname.startsWith('/api/ai/')) {
    ...
    if (m === 'POST' && pathname === '/api/ai/topic') {
      ...
      catch (e) { console.error('❌ AI topic xato:', e.code, e.message); return json(res, 502, { error: e.code || 'ai_error' }); }
