...
function emptyDb() {
  // groups ichida har bir guruhning g.chat massivi bo'ladi
  // attendance: har bir kun uchun bitta yozuv { id, groupId, date, marks:{studentId:true/false}, createdAt }
  return { seq: 1, users: {}, groups: [], requests: [], messages: [], attendance: [] };
}
...
const AI_SYS_HINT =
  "Siz \"To'garakga qatnashmaysizmi\" ilovasidagi foydali yordamchisiz. " +
  "Qisqa, aniq va samimiy javob bering. Foydalanuvchi qaysi tilda yozsa, o'sha tilda javob bering.";

// Gemini API bilan suhbat. history: [{role:'user'|'model', text}]
async function askGemini(history) {
  ...
  try {
    ...
    const r = await fetch(url, {
      ...
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        system_instruction: { parts: [{ text: AI_SYS_HINT }] },
        contents,
    ...
    if (j.error) { console.error('gemini:', j.error.message || j.error); return null; }
...
async function api(req, res, pathname, url, body) {
  ...
  if (pathname.startsWith('/api/admin/')) {
    ...
    if (m === 'POST' && pathname === '/api/admin/group') {
      if (body.action === 'delete') {
        ...
        db.attendance = (db.attendance || []).filter((a) => a.groupId !== String(body.id));
        save();
      }
      return json(res, 200, { ok: true });
    }
  ...
  if (m === 'POST' && pathname === '/api/group/update') {
    ...
    if (body.delete) {
      ...
      db.attendance = (db.attendance || []).filter((a) => a.groupId !== g.id);
      save();
      return json(res, 200, { ok: true });
    }
  ...
  if (m === 'POST' && pathname === '/api/ai') {
    ...
    const msgs = history
      ...
      .map((h) => ({ role: h.role, text: clean(h.text, 2000) }))
      .concat([{ role: 'user', text }]);
    const reply = await askGemini(msgs);
    if (reply == null) return json(res, 503, { error: 'ai_unavailable' });
    return json(res, 200, { reply });
  }

  /* ========================= DAVOMAT (ATTENDANCE) ========================= */

  /* Guruh davomatini olish — mentor: hamma a'zolar statistikasi; o'quvchi: faqat o'zinikini */
  if (m === 'GET' && pathname === '/api/attendance') {
    const groupId = String(url.searchParams.get('groupId') || '');
    const g = db.groups.find((x) => x.id === groupId);
    if (!g) return json(res, 404, { error: 'not_found' });
    const isMentor = g.mentorId === me.id;
    const isMember = g.members.includes(me.id);
    if (!isMentor && !isMember) return json(res, 403, { error: 'not_member' });
    ...
    if (isMentor) {
      const stats = g.members.map((sid) => {
        let present = 0, absent = 0;
        records.forEach((r) => {
          if (r.marks[sid] === true) present++;
          else if (r.marks[sid] === false) absent++;
        });
      ...
      return json(res, 200, { isMentor: true, records, stats });
    }
    ...
    const mine = records
      .map((r) => ({
        date: r.date,
        status: r.marks[me.id] === true ? 'present' : (r.marks[me.id] === false ? 'absent' : null),
      }))
      .filter((x) => x.status);
    mine.forEach((x) => (x.status === 'present' ? present++ : absent++));
    return json(res, 200, { isMentor: false, records: mine, present, absent, total: present + absent });
  }

  /* Davomat belgilash — faqat mentor, bitta kun uchun (qayta yuborilsa yangilanadi) */
  if (m === 'POST' && pathname === '/api/attendance') {
    const groupId = String(body.groupId || '');
    const g = db.groups.find((x) => x.id === groupId);
    if (!g || g.mentorId !== me.id) return json(res, 403, { error: 'mentor_only' });

    const date = /^\d{4}-\d{2}-\d{2}$/.test(String(body.date || '')) ? body.date : new Date().toISOString().slice(0, 10);
    const marksIn = body.marks && typeof body.marks === 'object' ? body.marks : {};
    const marks = {};
    for (const sid of g.members) {
      if (marksIn[sid] === true || marksIn[sid] === false) marks[sid] = marksIn[sid];
    }

    if (!db.attendance) db.attendance = [];
    let rec = db.attendance.find((a) => a.groupId === groupId && a.date === date);
    if (rec) rec.marks = marks;
    else { rec = { id: nextId(), groupId, date, marks, createdAt: now() }; db.attendance.push(rec); }
    save();
...
server.listen(PORT, () => {
  ...
  if (!GEMINI_API_KEY) console.log('⚠️  GEMINI_API_KEY qo\'yilmagan — AI yordamchi ishlamaydi.');
  else console.log(`🤖 AI yordamchi: ${GEMINI_MODEL} (Gemini) yoqilgan.`);
});
